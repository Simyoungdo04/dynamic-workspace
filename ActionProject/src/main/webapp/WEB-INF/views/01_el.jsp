...
<body>
	...
	<!-- 
		EL구문은 가장작은 범위의 scope부터 키값을 찾음
		page => request => session => application순으로 키 값을 검색
		
		모든 영역을 다 찾았는데 키가 존재하지 않는다면 => 아무일도 일어나지 않음 오류 X 
	 -->
	 
	 pageScope에 값이 담겨있는데, 다른 Scope값을 출력하고 싶을때 <br>
	 ...
	 <p>
	 	...
	 	big / small = ${ big / small } or ${ big div small }<br>
	 	big % small = ${ big % small } or ${ big mod small }<br>
	 </p>
	 ...
	 <p>
	 	big이 small보다 큰가? : ${ big > small } or ${ big gt small }<br>
	 	big이 small보다 작은가? : ${ big < small } ${ big lt small }<br>
	 	big이 small보다 크거나 같은가? : ${ big >= small } ${ big ge small }<br>
	 	big이 small보다 작거나 같은가? : ${ big <= small } ${ big le small }<br>
	 </p>
	 ...
	 <p>
	 	big과 small이 같습니까 ? : ${ big == small } or ${ big eq small } <br>
	 	big과 10이 같습니까 ? : ${ big eq 10 } or ${ big ne 10 } <br>
	 	
	 	one과 two가 같습니까 ? : ${ strOne == strTwo } or ${ strOne eq strTwo } <br>
	 	<!-- EL구문에서 문자열 == 비교는 equals()와 같은 동작을 함 -->
	 	문자열 비교하기 : ${ strOne == '안녕' } or ${ strOne == "안녕" } <br>
	 </p>
	 ...
	 <p>
	 	obj가 null인지 체크 <br>
	 	${ obj == null } or ${ obj eq null } or ${ empty obj } <br>
	 ...
	 <p>
	 	AND 연산 : ${ true && true } or ${ true and true } <br>
	 	OR 연산 : ${ true || true } or ${ true or true } <br>
	 </p>
