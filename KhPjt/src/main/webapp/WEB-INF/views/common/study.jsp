...
<body>
	...
	<pre>
		...
		우리가 앞에서 개발했던 방식을 동기방식
		
		동기방식 : 
		1. 사용자가 요청을 보냄
		2. 서버가 요청을 받아서 천체 HTML 데이터를 응답
		3. 브라우저는 응답받은 HTML데이터를 처음부터 끝까지 렌더링 => 전체 페이지를 다시 로딩 => 화면이 한 번 깜빡(SSR)
	...
	<pre>
		...
		사람이 일기 쉽고, 기계가 파싱하기 쉬운 데이터 교환형식 텍스트 기방이라 아주 가볍다
		
		* 진짜로 자바스크립트 객체 X / 자바스크립트 객체 모양으로 문자열을 만든거
		...
		{
			name : "name",		// 키에 따옴표 안 적어도 됨
			id : 'asdf****'		// 작은 따옴표 사용가능
			content : 'asdf',	// 마지막 속성에 컴마 가능
		}
	...
	응답 : <label id="output1">아직 응답없음</label>
		...
		function fn1(){
			...
			$.ajax({
				...
				error : result =>{
					...
					document.querySelector('#output1').innerHTML = "요청 실패";
				},
				complate : () => {
					console.log('성공 실패');
				}
