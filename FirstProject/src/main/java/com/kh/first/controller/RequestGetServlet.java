...
@WebServlet("/first.do") // Servlet == HTTP요청을 처리하는 Java클래스
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 3. 결과값 반환(응답화면 지정)
		 * 
		 * View에서 Get방식으로 요철할 시 doGet()가 호출됨
		 */
		/*
		 * 매개변수 두 개
		 * 
		 * 첫 번째 매개변수 HttpServletRequest타입에는 요청 시 전달된 내용들이 담김
		 * => 요청 전송 방식, 요청 URL, 요청한 사용자의 정보, 사용자가 input요소에 입력한 값 등..
		 * 
		 * 두 번째 매개변수 HttpServletResponse타입에는 요청 처리 후 응답할 떄 사용하는 객체
		 * 
		 * 요청 처리 스텝
		 * 
		 * 1. 요청을 처리하기 위해서 요청 시 전달된 값(사용자가 입력한 값)들을 뽑는다.
		 * => input요소의name속성값=input요소의value속성값
		 ...
		 * => 반환타입이 String / 다른 자료형으로 사용해야할 경우 parsing을 해주어야함
		 * 
		...
		// 404 : 파일 또는 요청을 받아주는 서브릿 매핑값을 찾지 못했을 때 발생
		//		=> 경로를 잘 못 적었거나 오타 났을 때
		...
		// INSERT라고 가정
		// Controller에서(여기) Service객체를 생성한 후 Service단의 메소드를 호출 하면서 Person객체의 주소갑을 전달 한 뒤
		// Service에서는 DAO단을 호출하여 DB SQL문(INSERT)를 수행한 뒤 -> 정수 형태의 결과값을 반환
